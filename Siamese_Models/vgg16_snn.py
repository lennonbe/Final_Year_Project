...
Original file is located at
    https://colab.research.google.com/drive/1oh0-7iKwYZdMGkcGhmauL9JLcBCM6T7o
"""
...
def minkowski_distance(x, y, p_value):
    return (p_root(tf.reduce_sum(tf.math.pow(tf.math.abs(a-b), p_value)).numpy()), p_value)
...
class L1Dist(Layer):
    ...
    def call(self, input_embedding, validation_embedding):
        #return minkowski_distance(input_embedding, validation_embedding, 3)
...
#vgg16_feature_extractor = Model(inputs=base.inputs, outputs=base.layers[-3].output)
...
vgg16_feature_extractor = Model(inputs=base.inputs, outputs=d1)
vgg16_feature_extractor.summary()
'''
...
def preprocess_twin(input_img, validation_img, label):
    return(tf.keras.applications.vgg16.preprocess_input(preprocess(input_img)), tf.keras.applications.vgg16.preprocess_input(preprocess(validation_img)), label)
...
opt = tf.keras.optimizers.Adam(1e-2) #0.01
...
def train(data, EPOCHS):
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        for idx, batch in enumerate(data):
            ...
            yhat = [1 if ((prediction > -0.20) and (prediction < 0.20)) else 0 for prediction in yhat]
...
plt.savefig('/content/drive/MyDrive/VGG16/Acc.png')
plt.show()
...
plt.savefig('/content/drive/MyDrive/VGG16/Precision.png')
plt.show()
...
plt.savefig('/content/drive/MyDrive/VGG16/Recall.png')
plt.show()
...
plt.savefig('/content/drive/MyDrive/VGG16/Loss.png')
plt.show()
...
for test_input, test_val, y_true in test_data.as_numpy_iterator():
    ...
    #Pre-processing results will lead to results closer 0 being more similar (matches to anchor) whilst more distant points will be further from 0 (not matches to anchor)
    yhat = [1 if ((prediction > -0.20) and (prediction < 0.20)) else 0 for prediction in yhat] #USED FOR TANH ACTIVATION OF DISTANCES
...
with open('/content/drive/MyDrive/VGG16/confusion_matrix.csv', 'w', newline ='') as csvfile:
